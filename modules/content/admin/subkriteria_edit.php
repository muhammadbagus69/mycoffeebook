<?php 
  ...
  $id = $_GET['id'];
  $query = $db->getRow("SELECT * FROM subkriteria WHERE id='$id'");
  ...
  if (!empty($_POST['subkriteria'])) 
  {
    ...
    $update = $db->Execute("UPDATE subkriteria SET id_kriteria='$kriteria',subkriteria='$subkriteria',nilai='$nilai' WHERE id='$id'");
    if ($update) 
    {
      echo "<script>alert('Ubah Berhasil')</script>";
      echo "<script>location.href='"._URL."admin/index.php?mod=content.subkriteria'</script>";
    }else{
...
<form class="form-horizontal" method="post" action="" role="form" enctype="multipart/form-data">
  <fieldset>
    ...
    <div class="form-group">
      ...
      <div class="col-lg-12">
        <input value="<?php echo $query['subkriteria']; ?>" class="form-control" id="subkriteria" placeholder="Nama Subkriteria" name="subkriteria" type="text" maxlength="50" required></div>
    </div>
    <div class="form-group">
